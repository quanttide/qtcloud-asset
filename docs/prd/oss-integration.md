...
| 访问链接生成 / 有效期控制 | 第一阶段不做；Plan A 后端门禁已临时落地默认 24h、最长 7 天 |
| 权限控制（管理员/内部/普通） | 第一阶段不做；Plan A 后端门禁已临时落地 `viewer` / `admin` 两级 |
| CDN 刷新、静态网站部署 | 提案「后续扩展方向」，非第一版 |
| GitHub / 飞书适配器 | 张果建议，属第二阶段（`SourceAdapter` 已预留接口） |

### 4.3 铁律

1. **`viewer` 不展示 `-private` 桶和 `quanttide-terraform-state`**；`admin` 可查看这些桶的元数据，但对象访问和签名链接仍走后端鉴权。
...
| 访问链接有效期与权限分级 | Plan A 临时决策 | 私密桶签名 URL 默认 24h，最长 7 天；30 天和永久链接暂不开放 |
