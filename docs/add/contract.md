...
1. **不管理文件列表** — 文件是动态变化的，契约管到文件级别会导致契约过于脆弱，每次增删文件都要改契约。
2. **抽象层次** — 契约回答“有什么资产”，由 `type`（docs/code/experiment）和 `category`（brd/prd/add/cli…）定义资产的分类和边界。
3. **扫描优于列举** — `git ls-files`、`find` 等工具天生就能发现文件，契约只需声明根路径和过滤规则。
